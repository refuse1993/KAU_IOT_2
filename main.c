...
#define GPIO_PULLUP (0x3 << GPIO_PUPD_SHIFT) //sonar
...
char device_id[] = 		"8ceb99bcdecb486a9e48174a7995fd6d";
char device_token[] = 		"202a87f8b04a4004906dfed88455bcd8";
...
#endif
{
    ...
    s5j_configgpio(cfgcon_echo);//sonar
	...
	printf("Temperature & Humidity (DHT22)\n");

	cfgcon_out = GPIO_OUTPUT | GPIO_PORTG2 | GPIO_PIN6;//xgpio22
	cfgcon_in = GPIO_INPUT | GPIO_PORTG2 | GPIO_PIN6;//xgpio22
	//tem,hum
    ...
    while (1) {
		...
		char buf[40];
		sprintf(buf, "{\"pdistance\":%.2f,\"tem\":%.2f,\"hum\":%.2f}",distance,temp,humi);
		printf("Send!!!\n");
		mqtt_msg_t message;
		message.payload = (char*)buf;/*data*/
		message.payload_len = strlen(buf);/*length*/
		message.topic = strTopicMsg;/*device id*/
		message.qos = 0;
		message.retain = 0;
